...
try:
    ...
    # CPU / non-CUDA fallback: pure-torch implementation of mamba-ssm rmsnorm_fn.
    # Slower than the Triton kernel but produces identical outputs and works on
    # CPU and AMD ROCm hosts where mamba-ssm is unavailable.
    def rmsnorm_fn(x, weight, bias=None, residual=None, eps=1e-6, prenorm=False,
                   residual_in_fp32=False, group_size=None, norm_before_gate=True,
                   is_rms_norm=True, **_kwargs):
...
class NemotronHBlock(nn.Module):
    ...
    def forward(
        ...
        # CPU / single-GPU fallback: torch.cuda.stream() is invalid off-CUDA.
        ...
        with _stream_ctx:
            ...
            if self.block_type == "mamba":
                ...
                # MoE forward now returns (hidden_states, router_logits) for the
                # REAP observer; the decoder only needs hidden_states.
...
class NemotronHMOE(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        topk_indices, topk_weights, router_logits = self.gate(
            hidden_states, return_logits=True
        )
        hidden_states = hidden_states.view(-1, hidden_states.shape[-1])
        hidden_states = self.moe(hidden_states, topk_indices, topk_weights).view(*orig_shape)
        hidden_states = hidden_states + self.shared_experts(residuals)
        # REAP observer contract: return raw router_logits alongside hidden states.
...
class NemotronHTopkRouter(nn.Module):
    ...
    def forward(self, hidden_states, return_logits: bool = False):
